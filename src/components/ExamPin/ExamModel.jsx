...
import { PurchaseData, PurchaseExamPin } from '../../store/apis/services';
...
const theme = Theme();
export default function ExamModal({ phone, data_plan }) {
  const { dataModel, dataPlans, user, token } = useSelector(
    (state) => state.globalState
  );
  const dispatch = useDispatch();
  const navigation = useNavigation();
  const { exam, quantity, amount } = useRoute().params;
  ...
  useEffect(() => {
    ...
    const DataList = [
...

      {
        id: 3,
        header: 'Amount',
        title: `₦${formatMoney(parseFloat(amount))}`,
      },
      {
        id: 4,
        header: 'Quantity',
  ...
  const handleSubmit = async ({ useBio }) => {
    ...
    const response = await PurchaseExamPin({
      quantity,
      token,
      provider: exam.nId,
      pin: pin.trim() === '' && currentPin ? currentPin : pin.trim(),
      ref: `EXAM_${GenerateRandomNumber()}_${Date.now()}`,
    });
