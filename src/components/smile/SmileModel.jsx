...
export default function SmileModel() {
  ...
  const { phone, account_type, plan: smilePlan } = useRoute().params;
  ...
  useEffect(() => {
    ...
    const DataList = [
      ...
      },
      {
        id: 2,
        header: 'Amount',
        title: 'N' + formatMoney(parseFloat(smilePlan?.price)),
      },
      {
        id: 3,
        header: 'Phone Number',
  ...
  const handleSubmit = async ({ useBio }) => {
    ...
    const response = await PurchaseSmile({
      token,
      phone,
      plan: smilePlan.BundleTypeCode,
      accountType: account_type,
      pin: pin.trim() === '' && currentPin ? currentPin : pin.trim(),
      ref: `SMILE_${GenerateRandomNumber()}${Date.now()}`,
    });
