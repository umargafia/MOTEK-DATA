...
import { SendAirtimeSwapRequest } from '../../store/apis/services';
...
const theme = Theme();
export default function AirtimeSwapModel({ phone, data_plan }) {
  const { dataModel, dataPlans, user, token } = useSelector(
    (state) => state.globalState
  );
  const dispatch = useDispatch();
  const navigation = useNavigation();
  const { amount, network, number, networkName, description } =
    useRoute().params;
  ...
  useEffect(() => {
    ...
    const DataList = [
      ...
      },
      {
        id: 2,
        header: 'Amount',
        title: 'N' + formatMoney(parseFloat(amount)),
      },
      {
        id: 3,
        header: 'Phone Number',
  ...
  const handleSubmit = async ({ useBio }) => {
    ...
    const response = await SendAirtimeSwapRequest({
      token,
      network,
      amount,
      pin,
      ...
      ref: `AIRTIME2CASH_${GenerateRandomNumber()}${Date.now()}`,
    });
