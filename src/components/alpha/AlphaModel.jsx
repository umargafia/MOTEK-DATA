...
export default function AlphaModel({}) {
  ...
  const { number, plan: currentPlan } = useRoute().params;
  ...
  const handleSubmit = async ({ useBio }) => {
    ...
    setLoading(true);
    const response = await PurchaseAlpha({
      phone: number,
      plan:
        user?.account_type.toLowerCase() === 'subscriber'
          ? currentPlan?.sellingPrice
          ...
          : currentPlan.vendor,
      token,
      ref: `ALPHA_${GenerateRandomNumber()}_${Date.now()}`,
      pin: pin.trim() === '' && currentPin ? currentPin : pin,
    });
    setLoading(false);
